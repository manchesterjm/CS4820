"""
Bidirectional Search implementation for the n-puzzle problem.

This module implements Bidirectional Search, which runs two simultaneous BFS
searches: one forward from start, one backward from goal. They meet in the middle
for O(b^(d/2)) efficiency instead of O(b^d).
"""
...
def bidirectional_bfs(puz: NPuzzle, start: Position):
    ...
    while f_front and b_front:
        ...
        if meet is not None:
            ...
            b_path = list(reversed(b_path))[1:]  # Reverse, skip duplicate
            path = f_path + b_path
