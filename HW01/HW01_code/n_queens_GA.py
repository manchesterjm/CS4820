"""
Genetic Algorithm implementation for the n-queens problem.

This module implements GA, which is inspired by biological evolution:
selection, crossover, and mutation. It maintains a population of solutions
that evolve over generations to find optimal placements.
"""
...
def select_parent(pop: List[List[int]], fit: List[int]) -> List[int]:
    ...
    - Balances exploitation (favor good solutions) and exploration
...
def mutate(ind: List[int], mut_rate: float) -> None:
    ...
    - For n-queens: ~0.05 works well (mutate a few columns)
...
def main():
    """
    Main experiment: Test 3 GA configs on 4-queens, pick best for 8-queens
    ...
    - Pick config with best performance (lowest time, then fewest gens)
    ...
    for i, cfg in enumerate(configs, 1):
        ...
        _, metrics = run_trials(4, cfg, trials_required=3, max_attempts=30)
    ...
    def score(metrics):
        avg_gens, _, avg_time = metrics
        return (avg_time, avg_gens)  # Tuple comparison: time first, gens
    ...
    print("="*40)
    cfg_str = f"pop={best_cfg[0]}, cx={best_cfg[1]}, "
    cfg_str += f"mut={best_cfg[2]}, gens={best_cfg[3]}"
    print(f"Config: {cfg_str}")
    print("Performance on n=4:")
    ...
    print("- Elitism ensures we never lose best solutions")
    print("- Fitness-proportionate selection balances pressure/diversity")
