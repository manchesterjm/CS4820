"""
A* Search implementation for the n-puzzle problem with two heuristics.

This module implements A* Search, an informed search algorithm that uses
f(n) = g(n) + h(n) where g(n) is cost to reach n, and h(n) is estimated
cost from n to goal. Includes two heuristics: misplaced tiles and Manhattan distance.
"""
...
def run_trials_auto():
    ...
    print("    (each misplaced tile needs >=1 move)")
    ...
    print("  - Consistent: Moving a tile changes Manhattan distance by +/-1")
    print("  - Dominates h_misplaced: h_manhattan(n) >= h_misplaced(n)")
    print("  - Strong heuristic: Better estimates -> fewer nodes expanded")
    print("  - Result: Usually faster despite more complex computation")
