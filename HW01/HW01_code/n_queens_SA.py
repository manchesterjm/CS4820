...
This module implements SA, a probabilistic local search that accepts worse
moves to escape local optima. It is inspired by the metallurgical annealing
process of heating and slow cooling.
"""
...
def simulated_annealing(n: int,
    ...
    - Temperature controls acceptance probability: P(accept) = e^(dE/T)
    ...
    - If dE < 0 (worse): accept with probability e^(dE/T)
    - Example: dE=-1, T=1.0 -> P=0.368 (36.8% chance)
    - Example: dE=-1, T=0.1 -> P=0.000045 (rare)
    ...
    for _ in range(1, max_iters+1):
        ...
        # Calculate change in value: dE = value(next) - value(current)
        # Positive dE means improvement (fewer conflicts)
        dE = nxt_h - cur_h
        ...
        if dE > 0:
            ...
            # P(accept) = e^(dE/T)
            # - Large negative dE (much worse) -> low probability
...
def run_trials_auto():
    ...
    SA parameter tuning:
    - n=4 (small): Lower temp, faster cooling (converges quickly)
    - n=8 (larger): Higher temp, slower cooling (needs more exploration)

    Temperature guidelines:
    - T0 ~ average dE for typical bad move (problem-dependent)
    - Cooling rate: 0.95-0.999 (slower = more thorough but more time)
    - For n-queens: conflicts change by small amounts, so T0=1-2 works
    ...
    for n, T0, cooling, max_iters in configs:
        ...
        param_str = f"Parameters: T0={T0}, cooling={cooling}, "
        param_str += f"max_iters={max_iters}"
        print(param_str)
        print("="*36)
        ...
        while successes < TRIALS_REQUIRED and attempts < MAX_ATTEMPTS_PER_SIZE:
            attempts += 1
            print(f"\nattempt {attempts}:")
            ok, expanded, dt, _ = run_once(n, T0, cooling, max_iters)
            ...
            if ok:
                ...
                # Adaptive strategy: slow down cooling for success
                # Slower cooling = more iterations at each temperature
                # = more thorough exploration
                cooling = min(0.9995, cooling + 0.0005)
                print(f"  (adjusting cooling to {cooling} for exploration)")
